<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('model.updateevalution', ['modelinfo' => $model->id]) }}" method="post">
            @csrf
            <table class="w-full border-collapse border border-blue-600 rounded-md">
                <thead>
                    <tr class="bg-blue-600">
                        <th class="py-2 px-4 border text-white">Evaluation</th>
                        <th class="py-2 px-4 border text-white">Number/Time</th>
                        <th class="py-2 px-4 border text-white">Weight (Marks)</th>
                        <th class="py-2 px-4 border text-white">Week Due</th>
                        <th class="py-2 px-4 border text-white">Relevant Learning Outcome</th>
                        <th class="py-2 px-4 border text-white">Action</th>
                    </tr>
                </thead>
                <tbody id="tbody">
                    ...
                    <tr class="border border-blue-600">
                        <td class="py-2 px-4 border"><input type="text" name="evaluation[]"
                                ...
                                class="w-full border border-blue-300 rounded-md py-1 px-2"></td>
    ...
    <script>
        document.querySelector('.btn-add').addEventListener('click', function() {
            ...
            newRow.innerHTML = `
                <td class="py-2 px-4 border"><input type="text" name="evaluation[]" class="w-full border border-blue-300 rounded-md py-1 px-2"></td>
                <td class="py-2 px-4 border"><input type="text" name="number_time[]" class="w-full border border-blue-300 rounded-md py-1 px-2"></td>
                <td class="py-2 px-4 border"><input type="text" name="weight_mark[]" class="w-full border border-blue-300 rounded-md py-1 px-2"></td>
                <td class="py-2 px-4 border"><input type="text" name="week_due[]" class="w-full border border-blue-300 rounded-md py-1 px-2"></td>
                <td class="py-2 px-4 border"><input type="text" name="relevant_learning_outcome[]" class="w-full border border-blue-300 rounded-md py-1 px-2"></td>
                <td class="py-2 px-4 border">
