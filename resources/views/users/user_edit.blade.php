...
<div class="flex items-center justify-center h-screen ">
    <div class="  bg-blue-600 p-6 rounded-lg shadow-md w-3/4 h-3/4 flex justify-center items-center">
      <form action="{{ route('user.update', $user->id) }}" method="POST" class="mt-8 space-y-6 w-3/4 flex-col justify-center items-center">
        @csrf
        <h2 class="text-center text-4xl font-semibold mb-6 text-white">Change Role</h2>
            ...
            <div class="mb-4">
                ...
                <select name="role_id"
                  ...
                  <option value="1" {{ $user->role_id == 1 ? 'selected' : '' }}>Head Of Department</option>
                  <option value="2" {{ $user->role_id == 2 ? 'selected' : '' }}>Admin</option>
                </select>
                
            </div>
            <div class="flex justify-center">
              <button type="submit" class="mt-4 font-bold text-xl bg-white text-blue-600 px-4 py-2 rounded-md hover:bg-blue-100 focus:outline-none focus:bg-blue-600">Change Role</button>
            </div>
        </form>
    </div>
    </div>
